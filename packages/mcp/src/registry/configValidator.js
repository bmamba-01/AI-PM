...
export function validateConfigs(registry, profiles = []) {
    const issues = [];
...
function validateGlobalApprovalPolicy(registry, issues) {
    const hasMutationCapabilities = Object.values(registry.servers).some(s => s.mutation_capabilities.length > 0);
    if (!hasMutationCapabilities)
        return;
    ...
    if (defaultPolicy !== "approval_required" && globalOverride !== true) {
        issues.push({
            ...
            message: "Global mutation policy is not set to approval_required, but connectors declare mutation capabilities.",
            context: {
...
function validateContractReferences(registry, issues) {
    ...
    const existingFiles = new Set(fs
...
function validateWorkflowServerReferences(registry, issues) {
    ...
    for (const server of Object.values(registry.servers)) {
        const missingCapabilities = server.read_capabilities.filter(cap => typeof cap !== "string" || cap.trim() === "");
        const missingMutations = server.mutation_capabilities.filter(cap => typeof cap !== "string" || cap.trim() === "");
...
function validateProfileReferences(registry, profiles, issues) {
    ...
    for (const profile of profiles) {
        ...
        const workflowExpectations = profile
            .workflow_expectations;
        if (!workflowExpectations)
            continue;
