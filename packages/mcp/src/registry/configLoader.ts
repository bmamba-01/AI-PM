...
export function loadRegistry(registryPath = DEFAULT_REGISTRY_PATH): RegistryConfig {
  const raw = fs.readFileSync(registryPath, "utf-8");
  const parsed = yaml.parse(raw) as RegistryConfig;
...
export function loadProfile(profilePath: string): ProfileConfig {
