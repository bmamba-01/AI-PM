...
describe('applyProfileDefaults', () => {
  it('fills all defaults for empty input', () => {
    ...
    expect(p.artifacts).toEqual({ root: '.', reports: 'reports', templates: 'templates', notes: 'notes' });
    expect(p.approval_policy).toBeDefined();
    expect(p.approval_policy!.require_approval_for_email).toBe(true);
  });
...
describe('validateProfile', () => {
  ...
  it('rejects invalid methodology', () => {
    ...
    expect(result.valid).toBe(false);
    expect(result.errors.some(e => e.includes('methodology'))).toBe(true);
  });
  ...
  it('rejects non-boolean approval_policy values', () => {
    ...
    expect(result.valid).toBe(false);
    expect(result.errors.some(e => e.includes('approval_policy'))).toBe(true);
  });
  ...
  it('applies defaults to partial profile', () => {
    const result = validateProfile({
      version: 1,
      project: { project_id: 'p1', name: 'Test', root: '.' },
    });
    expect(result.profile.project.timezone).toBe('UTC');
    expect(result.profile.artifacts!.reports).toBe('reports');
    expect(result.profile.approval_policy!.require_approval_for_email).toBe(true);
  });
});
