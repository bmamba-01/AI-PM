function statusCounts(risks) {
    const counts = { open: 0, mitigating: 0, closed: 0, accepted: 0 };
    for (const risk of risks)
        counts[risk.status] = (counts[risk.status] || 0) + 1;
    return counts;
}
function probabilityCounts(risks) {
    const counts = { low: 0, medium: 0, high: 0 };
    for (const risk of risks)
        counts[risk.probability] = (counts[risk.probability] || 0) + 1;
    return counts;
}
function impactCounts(risks) {
    const counts = { low: 0, medium: 0, high: 0, critical: 0 };
    for (const risk of risks)
        counts[risk.impact] = (counts[risk.impact] || 0) + 1;
    return counts;
}
...
export function generateRiskControlSummary(input) {
